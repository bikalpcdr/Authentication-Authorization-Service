...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken=getAuthentiction(request);
        SecurityContextHolder.getContext().setAuthentication(usernamePasswordAuthenticationToken);
        chain.doFilter(request,response);
    }

    private UsernamePasswordAuthenticationToken getAuthentiction(HttpServletRequest request){
        String header = request.getHeader(SecurityUtilsConstant.HEADER_STRING);
        return (UsernamePasswordAuthenticationToken) tokenService.getAuthenticationByToken(header);
    }
